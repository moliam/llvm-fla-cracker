...
# the input file's format should be exact the form of the pseudocode generated by IDA. What's more, all the number appeared in the comparision should be in decimal. 
# For example, "if ( v1 > 0x20 )" won't work and it should be "if ( v1 > 32 )" 



#-------------------------Some constants that are intended to help remember the array. 
...
#used to parse the result from WordScan()
LEFT_ALL = 2
LEFT_RELATION = 2
LEFT_RELVALUE = 3
LEFT_VARNAME = 1
...
#used during build relation
VALUE_IN = 0
VALUE_OUT = 1
USE = 2


#----trace property. the cracker records the property of every line it simulates.
TP_SIMOUT = "SimOut"
TP_CTRLVAR_DECL = ""
TP_CTRLVAR_SET = ""
TP_CTRL = "Ctrl"
...
TP_LBL_OUT = "LOut"  # "{"
TP_LBR_OUT = "ROut"  # "}"
TP_NEW_BR = "NEW_Br"   # new branch

#---- the info of the result of WordScan
SCANTYPE_CTRL = "ctrl"
...
#----------ctrl stack (the stack structure used during simulating the code)
CONTROL_NAME = 0
#---- the type of the items in control stack
CSTK_DO = "do"
CSTK_WHILE = "while"
CSTK_GOTO = "goto"
CSTK_IF = "if"
...
GlobalBranchId = 0  # for labeling the if else branch. 
GlobalLoopId = 0    #for labeling the found loop
...
#-----------------------------------------------check loop
		if Stat in Trace:  # a loop found
			...
			Loop.append(["Loop_" + LoopId, fromLineIdx, toLineIdx])
			...
			if ScanInfo[1] == SCAN_IF or ScanInfo[1] == SCAN_ELSEIF:
				...
				if var in VarSet:     # condition can be resolved. 
					...
					if ScanInfo[1] == SCAN_IF and not SensVarRelatedinIf(LineBuf, lineidx, VarSet):
						...
						# @@@@@@@@@@@@@@@@@  RECURSIVELY simulate the rest of the code.
			...
			if ScanInfo[1] == SCAN_BREAK:
				...
				while 1:
					...
					if CtrlLabel[CONTROL_NAME] == CSTK_WHILE or CtrlLabel[CONTROL_NAME] == CSTK_SWITCH or CtrlLabel[CONTROL_NAME] == CSTK_DO:
						lineidx = FindNextBlock(LineBuf, CtrlLabel[LINE_IDX])
...
#*******************
		...
		if HeadType != SCANTYPE_CTRL and CtrlStack != []:
			if CtrlStack[-1][CONTROL_NAME] == CSTK_IF:
				lineidx = SkipIf(LineBuf, CtrlStack[-1][LINE_IDX])
				CtrlStack.pop()
			elif CtrlStack[-1][CONTROL_NAME] == CSTK_WHILE:
				lineidx = CtrlStack[-1][LINE_IDX]  # repeat
				CtrlStack.pop()
